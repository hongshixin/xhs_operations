...
def _live_photo_urls(normalized: dict[str, Any]) -> list[str]:
    urls = normalized.get("live_photo_urls")
...
def _save_normalized_notes(
    ...
    for normalized in normalized_items:
        ...
        for url in _live_photo_urls(normalized):
            local_name = _download_asset(url, account.user_id, "video")
            db.add(NoteAsset(note_id=note.id, asset_type="live_photo", url=url, local_path=local_name or ""))
