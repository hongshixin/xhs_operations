...
def _live_photo_urls(card: dict[str, Any]) -> list[str]:
    """提取图集中每张图的 Live Photo 动图地址，无动图的位置填空字符串"""
    ...
    for img in image_list:
        ...
        h264 = stream.get("h264") or [] if isinstance(stream, dict) else []
        url = ""
        if isinstance(h264, list) and h264:
            first = h264[0] if isinstance(h264[0], dict) else {}
            url = str(first.get("master_url") or first.get("url") or "")
        if not url:
            url = str(img.get("live_photo") or img.get("livePhoto") or "")
        result.append(url)
    return result
...
def _normalize_detail_payload(raw_payload: dict[str, Any], source_url: str = "") -> dict[str, Any]:
    ...
    normalized["live_photo_urls"] = _live_photo_urls(card)
