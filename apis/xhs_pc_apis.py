...
from xhs_utils.http_util import REQUEST_TIMEOUT, xhs_get, xhs_post
...
class XHS_Apis():
    ...
    def get_homefeed_recommend(self, category, cursor_score, refresh_type, note_index, cookies_str: str, proxies: dict = None):
        ...
        try:
            ...
            response = xhs_post(self.base_url + api, headers=headers, data=trans_data, cookies=cookies, proxies=proxies, timeout=REQUEST_TIMEOUT)
            res_json = response.json()
            success, msg = res_json["success"], res_json["msg"]
        except Exception as e:
            success = False
    ...
    def get_note_info(self, url: str, cookies_str: str, proxies: dict = None):
        ...
        try:
            ...
            response = xhs_post(self.base_url + api, headers=headers, data=data, cookies=cookies, proxies=proxies, timeout=REQUEST_TIMEOUT)
            res_json = response.json()
            success, msg = res_json["success"], res_json["msg"]
        except Exception as e:
            success = False
    ...
    def search_note(self, query: str, cookies_str: str, page=1, sort_type_choice=0, note_type=0, note_time=0, note_range=0, pos_distance=0, geo="", search_id=None, proxies: dict = None):
        ...
        try:
            ...
            headers, cookies, data = generate_request_params(cookies_str, api, data, 'POST')
            headers["x-rap-param"] = generate_x_rap_param(api, data)
            response = xhs_post(self.base_url + api, headers=headers, data=data.encode('utf-8'), cookies=cookies, proxies=proxies, timeout=REQUEST_TIMEOUT)
            res_json = response.json()
            success, msg = res_json["success"], res_json["msg"]
        except Exception as e:
            success = False
    ...
    def get_note_no_water_video(note_id):
        ...
        try:
            ...
            response = xhs_get(url, headers=headers, timeout=REQUEST_TIMEOUT)
            res = response.text
